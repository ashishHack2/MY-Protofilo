        ...
        <div className="space-y-12 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-brand-teal before:via-brand-violet/50 before:to-transparent">
          {experience.map((job, idx) => (
            <div key={job.company} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
              {/* Timeline dot */}
              <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#0D1117] bg-brand-teal text-[#0D1117] absolute left-0 md:left-1/2 -translate-x-1/2 shrink-0 md:order-1 shadow-[0_0_15px_rgba(42,157,143,0.5)] z-10 transition-transform group-hover:scale-110">
                <Briefcase size={18} />
              </div>
              ...
              >
                <div className="flex flex-col mb-4">
                  <h3 className="font-display text-2xl font-bold text-white group-hover:text-brand-teal transition-colors">{job.role}</h3>
                  <div className="flex flex-wrap justify-between items-center mt-1 gap-2">
                    <span className="text-brand-violetLight font-medium text-lg">{job.company}</span>
                    <span className="font-mono text-xs text-brand-teal bg-brand-teal/10 px-3 py-1 rounded-full">{job.duration}</span>
                  </div>
                </div>

                <ul className="space-y-3 mb-6">
                  {job.description.map((desc, i) => (
                    <li key={i} className="flex items-start gap-3 text-brand-text/90 text-sm md:text-base leading-relaxed">
                ...
                <div className="flex flex-wrap gap-2">
                  {job.tech.map((t) => (
                    <span key={t} className="font-mono text-xs text-brand-muted bg-brand-bg px-2 py-1 rounded-md border border-brand-violet/20">
                      {t}
                    </span>
                  ))}
                </div>
